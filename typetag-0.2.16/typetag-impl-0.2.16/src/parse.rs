...
use syn::{Attribute, Error, ImplItem, ItemImpl, ItemTrait, LitStr, Token, TraitItem, Type, Visibility};
...
pub struct TraitArgs(pub Option<Ident>, pub Tag );
pub enum Tag{
    External,
    Internal {
        tag: LitStr,
...
impl Parse for TraitArgs {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if lookahead.peek(kw::receiver) {
            input.parse::<kw::receiver>()?;
            input.parse::<Token![=]>()?;
            receiver = Some(input.parse()?);
            if !input.is_empty() {
                input.parse::<Token![,]>()?;
            }
        } else {
        ...
        if input.is_empty() {
            return Ok(TraitArgs(receiver, Tag::External));
        }
        input.parse::<kw::tag>()?;
        input.parse::<Token![=]>()?;
        ...
        if input.is_empty() {
            return Ok(TraitArgs(receiver, Tag::Internal {
                tag,
        ...
        if lookahead.peek(kw::content) {
            ...
            Ok(TraitArgs(receiver,Tag::Adjacent {
                tag,
                content,
                default_variant,
                deny_unknown_fields,
