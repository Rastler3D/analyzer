...
use syn::{parse_quote, Error, ItemImpl, Type, TypePath, parse_macro_input};
...
pub(crate) fn expand(args: ImplArgs, mut input: ItemImpl, mode: Mode) -> TokenStream {
    ...
    let struct_name = format_ident!("{}Registry", object.to_token_stream().to_string());
    if mode.de {
        expanded.extend(quote! {
            typetag::__private::inventory::submit! {
                #struct_name::typetag_register(
                    #name,
                    ...
                    (|gen| gen.subschema_for::<#this>()) as typetag::__private::SchemaFn,
                )
            }
        });
    }
